...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
      ...
      .insert([
        {
          ...
          user_address: req.body.userAddress || 'anonymous'
        }
      ])
      .select();
    ...
    if (error) {
      ...
      // Fallback: just return the file info
      res.json({
        success: true,
        contentHash,
      ...
      res.json({
        ...
        fileSize: req.file.size,
        id: data[0].id,
        message: 'File uploaded and registered in database! 🎉'
      });
    }
...
app.listen(port, () => {
  ...
  console.log(`💾 Database: Supabase (configure your credentials)`);
});
